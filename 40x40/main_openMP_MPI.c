...
int main(int argc, char *argv[]) {
    ...
    int row_start = (M/n - 1) * tup_x[0];
    int col_start = (N/k - 1) * tup_x[1];
    int row_end = M/n - 1 + tup_x[0]*(M/n) + (n-1)*(!tup_x[0]);
    ...
    for (iter = 0; iter < INT_MAX; iter++)
    {
        ...
        #pragma omp parallel for collapse(2) reduction(+:r_r) reduction(max:max_r) private(i, j) shared(a, b, F, r, w, top_send_mas, down_send_mas, left_send_mas, right_send_mas)
      ...
      //  }
        ...
        #pragma omp parallel for collapse(2) reduction(+:Ar_r) private(i, j) shared(a, b, F, r, w, Ar)
        for (i = row_start + 1; i < row_end; i++) {
            for (j = col_start + 1; j < col_end; j++) {
        ...
        #pragma omp parallel for collapse(2) private(i, j) shared(a, b, F, r, w, w_rotated, top_send_mas, down_send_mas, left_send_mas, right_send_mas)
        for (i = row_start + 1; i < row_end; i++) {
            for (j = col_start + 1; j < col_end; j++) {
                ...
                if(i == row_start + 1 && tup_x[0]){
                    ...
                    top_send_mas[j - col_start - 1] = w[i][j];
                }
                if(i == row_end - 1 && tup_x[0] + 1 < n){
                    // для отправки второй строке
                    down_send_mas[j - col_start - 1] = w[i][j];
                }
                if(j == col_start + 1 && tup_x[1]){
                    // для отправки первому столбцу при наличии столбца 2
                    left_send_mas[i - row_start - 1] = w[i][j];
                }
                if(j == col_end - 1 && tup_x[1] + 1 < k){
                    // для отправки второму столбцу
                    right_send_mas[i - row_start - 1] = w[i][j];
                }
            }
        }
